    ...
    def get_symbol_galleries(cls):
        if not cls.search_results or len(cls.search_results) == 0:
            ...
            if len(cls.search_results) > cls.search_limit:
                image_msos.append(
                    bkt.ribbon.Button(label="Max. {} icons per page".format(cls.search_limit), enabled=False)
                )
            ...
            ###### for group, icons in cls.search_results.sortedby("name").groupedby("module", 999).limit(cls.search_limit):
                for page, icons in enumerate(cls.search_results.paginate(cls.search_limit), start=1):
                    image_msos.append(
                        bkt.ribbon.Menu(
                            label="Page {} ({}..-{}..)".format(page, icons[0].keywords[:3].capitalize(), icons[-1].keywords[:3].capitalize()),
                            children=[
                                bkt.ribbon.Button(
                                    label=icon.name,
