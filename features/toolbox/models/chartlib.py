...
class ChartLib(object):
    ...
    def get_root_menu(self):
        ...
        if self.library_files:
            menu.children.append( bkt.ribbon.MenuSeparator(title="Individual libraries") )
        ...
        menu.children += [
            ...
            bkt.ribbon.Button(label="Re-index library",
                screentip="Re-index library",
                supertip="All caches are reset. Library folders and files are each re-indexed the next time they are opened.\n\nOnly thumbnails of already-opened libraries are updated. Thumbnails can also be regenerated separately for each shape-library file (in the respective menu).",
            ...
            bkt.ribbon.DynamicMenu(
                ...
                supertip="Add or remove library files or folders.",
    ...
    def get_template_file_control(self, filename):
        ...
        return bkt.ribbon.Menu(
            ...
            children=[
                bkt.ribbon.Button(
                    label="Apply design to selected slides",
                    supertip="Apply slide master to the selected slides.",
                    tag=filename,
                    on_action=bkt.Callback(self.apply_template_to_slides, current_control=True, context=True)
                ),
                bkt.ribbon.Button(
                    label="Apply design to the entire presentation",
                    supertip="Apply slide master to the entire presentation.",
                    tag=filename,
                    on_action=bkt.Callback(self.replace_template_in_presentation, current_control=True, context=True)
                ),
                bkt.ribbon.Button(
                    label="Add design to presentation",
                    supertip="Add slide master to the current presentation.",
                    tag=filename,
                    on_action=bkt.Callback(self.apply_template_to_slides, current_control=True, context=True)
                ),
                bkt.ribbon.Button(
                    label="Create new presentation with design",
                    supertip="Create a new empty presentation based on the slide master.",
                    tag=filename,
                    on_action=bkt.Callback(self.new_presentation_with_template, current_control=True, context=True)
                ),
                bkt.ribbon.MenuSeparator(),
                bkt.ribbon.Button(label="Open file and edit",
    ...
    def get_libraries_menu(self):
        children = []
        if self.library_files:
            children.append( bkt.ribbon.MenuSeparator(title="Individual libraries") )
            for filename in self.library_files:
                children.append(
                    bkt.ribbon.Button(
        ...
        if self.library_folders:
            ...
            if folders_lib:
                children.append(bkt.ribbon.MenuSeparator(title="Folder libraries"))
                children.extend(folders_lib)
            if folder_feat:
                children.append(bkt.ribbon.MenuSeparator(title="About feature folders"))
                children.extend(folder_feat)

        children.extend([
            bkt.ribbon.MenuSeparator(title="Add"),
            bkt.ribbon.Button(
                label="Add file to library",
                supertip="Select library files and add them permanently to the configuration.",
                image_mso='FilesToolAddFiles',
                on_action=bkt.Callback(self.add_files_to_config)
            ),
            bkt.ribbon.Button(
                label="Add folder to library",
                supertip="Select a library folder and add it permanently to the configuration.",
                image_mso='Folder',
                on_action=bkt.Callback(self.add_folders_to_config)
            ),
            bkt.ribbon.Button(
                label="Create new library file",
                supertip="Select a library folder and add it permanently to the configuration.",
                image_mso='FileSaveAsPowerPointPptx',
    ...
    def get_file_buttons(self, filename, show_menu_separator=True):
        return ([bkt.ribbon.MenuSeparator(title = "Library")] if show_menu_separator else []) + [
            bkt.ribbon.Button(label="Open file and edit library",
                image_mso='FileSaveAsPowerPointPptx',
                tag=filename,
                on_action=bkt.Callback(self.open_file, context=True, current_control=True)
            ),
            bkt.ribbon.Button(label="Re-index file",
                image_mso='AccessRefreshAllLists',
                tag=filename,
...
class ChartLibGallery(bkt.ribbon.Gallery):
    ...
    def get_file_buttons(self):
        return [
            ...
            bkt.ribbon.Button(label="Re-index file and update thumbnails",
                image_mso='AccessRefreshAllLists',
