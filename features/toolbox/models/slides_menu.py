...
slides_menu = memoize(lambda: bkt.ribbon.Menu(
                ...
                children=[
                    ...
                    bkt.ribbon.Button(
                        ...
                        label="Replace slide master from file…",
                        image_mso="SlideMasterMasterLayout",
                        supertip="Replaces the current slide master (templates and design) in the presentation with the slide master from the selected file.",
                    ...
                    bkt.ribbon.Button(
                        id = 'slide_no',
                        label='Show/hide slide numbering',
                        image_mso='NumberInsert',
                        #screentip="Show/hide slide numbering",
                        supertip="Insert slide numbering that does not change when slides are reordered.\n\nHelpful for capturing notes when reordering during a discussion of the slide deck.",
                        on_action=bkt.Callback(SlideMenu.toggle_slide_numbering)
                    ),
                    bkt.ribbon.Button(
                        id = 'save_slides',
                        label='Save selected slides',
                        image_mso='SaveSelectionToTextBoxGallery',
                        supertip="Saves the selected slides in a new presentation, as a PDF file or in PNG images.",
                        on_action=bkt.Callback(SlideMenu.save_slides_dialog)
                    ),
                    bkt.ribbon.Button(
                        id = 'send_slides',
                        label='Send selected slides',
                        image_mso='FileSendAsAttachment',
                        supertip="Sends the selected slides as an e-mail attachment, optionally also as a PDF file.",
                        on_action=bkt.Callback(SlideMenu.send_slides_dialog)
                    ),
                    bkt.ribbon.Button(
                        id = 'sync_slides',
                        label='Match selected slides',
                        image_mso='SlideShowShowPresentationOnGallery',
                        supertip="Matches all shapes on the slides to the first selected slide based on the shape name. This function is useful when you create a template, duplicate it multiple times and want to synchronize changes afterwards",
                        on_action=bkt.Callback(SlideMenu.sync_slides_dialog)
                    ),
                    bkt.ribbon.SplitButton(children=[
                        bkt.ribbon.Button(
                            id = 'slide_remove_all',
                            label='Clean up slide deck…',
                            image_mso='SlideShowFromCurrent', #AcceptTask, SlideShowFromCurrent, FilePublishSlides
                            supertip="Shows a dialog to select the functions to apply.",
                            on_action=bkt.Callback(SlideMenu.remove_all)
                        ),
                        bkt.ribbon.Menu(label="Clean up slide deck", supertip="Functions to clean up all slides of the presentation", image_mso='SlideShowFromCurrent', children=[
                            bkt.ribbon.Button(
                            ...
                            bkt.ribbon.MenuSeparator(title="Contents"),
                            bkt.ribbon.Button(
                                id = 'slide_remove_hidden_slides',
                                label='Remove hidden slides',
                                image_mso='SlideHide',
                                supertip="Delete all hidden slides in the entire slide deck.",
                            ...
                            bkt.ribbon.Button(
                                ...
                                supertip="Delete all notes in the entire slide deck.",
                            ...
                            bkt.ribbon.Button(
                                ...
                                supertip="Delete all comments in the entire slide deck.",
                            ...
                            bkt.ribbon.Button(
                                ...
                                supertip="Remove author from the document properties.",
                            ...
                            bkt.ribbon.Button(
                                ...
                                supertip="Removes the link of linked objects (e.g. images and OLE objects).",
                                on_action=bkt.Callback(SlideMenu.break_links)
                            ),
                            bkt.ribbon.MenuSeparator(title="Animations"),
                            bkt.ribbon.Button(
                                id = 'slide_remove_transitions',
                                label='Remove slide transitions',
                                image_mso='AnimationTransitionGallery',
                                supertip="Delete all transitions between slides.",
                                on_action=bkt.Callback(SlideMenu.remove_transitions)
                            ),
                            bkt.ribbon.Button(
                                id = 'slide_remove_animation',
                                label='Remove shape animations',
                                image_mso='AnimationGallery',
                                supertip="Delete all shape animations in the entire slide deck.",
                                on_action=bkt.Callback(SlideMenu.remove_animations)
                            ),
                            bkt.ribbon.MenuSeparator(title="Clean up format"),
                            bkt.ribbon.Button(
                                id = 'slide_grayscale',
                                label='Disable automatic black/white mode',
                                image_mso='BlackAndWhiteGrayscale',
                                supertip="Replace the black/white mode 'Automatic' with 'Grayscale'.",
                            ...
                            bkt.ribbon.Button(
                                ...
                                supertip="Delete all double spaces in the entire slide deck.",
                                on_action=bkt.Callback(SlideMenu.remove_doublespaces)
                            ),
                            bkt.ribbon.Button(
                                id = 'slide_remove_empty_placeholders',
                                label='Remove empty placeholders',
                                image_mso='HeaderFooterRemoveHeaderWord',
                                supertip="Delete empty placeholder text boxes in the entire slide deck.",
                                on_action=bkt.Callback(SlideMenu.remove_empty_placeholders)
                            ),
                            bkt.ribbon.MenuSeparator(title="Slide master"),
                            bkt.ribbon.Button(
                                id = 'slide_remove_unused_masters',
                                label='Remove unused slide layouts',
                                image_mso='SlideDelete',
                                supertip="Delete all unused slide-master layouts as well as empty slide masters (designs).",
                                on_action=bkt.Callback(lambda context: SlideMenu.remove_unused_masters(context, False))
                            ),
                            bkt.ribbon.Button(
                                id = 'slide_remove_unused_designs',
                                label='Remove unused slide masters',
                                image_mso='SlideDelete',
                                supertip="Delete all unused slide masters (designs).",
                                on_action=bkt.Callback(lambda context: SlideMenu.remove_unused_designs(context, False))
                            ),
                        ]),
                    ]),
                    language.sprachen_menu,
                    bkt.ribbon.MenuSeparator(title="View"),
                    bkt.ribbon.Menu(label="Master views", supertip="Switch between different master views", image_mso='GroupPresentationViews', children=[
                    ...
                    bkt.ribbon.Menu(label="Color/grayscale", supertip="Switch between different color modes", image_mso='ColorGrayscaleMenu', children=[
                    ...
                    bkt.ribbon.Button(
                        label="Show/hide template shapes",
                        image_mso='SlideHide',
                        supertip="Show and hide all shapes in the slide master to work on slides undisturbed and confidentially.",
                        on_action=bkt.Callback(SlideMenu.toggle_hide_master_shapes)
                    ),
                    bkt.ribbon.Button(
                        label="Open in Explorer",
                        image_mso='OpenFolder',
                        supertip="Open the folder of the current presentation in Explorer.",
                        on_action=bkt.Callback(SlideMenu.open_in_explorer)
                    ),
                    bkt.ribbon.MenuSeparator(title="Slide show"),
                    bkt.ribbon.Button(
                        id="slide_windowed_slideshow",
                        image_mso="SlideShowInAWindow",
                        label="Start in window",
                        supertip="Starts a slide show in a window beginning with the current slide and the laser pointer activated.",
                    ...
                    bkt.ribbon.Button(
                        ...
                        supertip="Starts a slide show in full screen beginning with the current slide and the laser pointer activated.",
