...
class NumberShapesGallery(bkt.ribbon.Gallery):
    ...
    def __init__(self, **kwargs):
        ...
        my_kwargs = dict(
            ...
            screentip="Insert numbering shapes",
            supertip="Inserts a numbering shape for each selected shape. Numbering and styling according to the selection. Selected shapes are numbered according to the selection order.",
            get_image=bkt.Callback(lambda: self.get_item_image(0)),
            get_enabled = bkt.apps.ppt_shapes_or_text_selected,
            item_width=24,
            item_height=24,
            children=[
                bkt.ribbon.Button(id=parent_id + "_pos_left", label="Position top left", screentip="Numbering shapes top-left",    on_action=bkt.Callback(self.set_pos_top_left), get_image=bkt.Callback(lambda: self.get_toggle_image('pos-top-left')),
                    supertip="Place numbering shapes at the top left of the associated shape"),
                bkt.ribbon.Button(id=parent_id + "_pos-right", label="Position top right", screentip="Numbering shapes top-right", on_action=bkt.Callback(self.set_pos_top_right), get_image=bkt.Callback(lambda: self.get_toggle_image('pos-top-right')),
                    supertip="Place numbering shapes at the top right of the associated shape"),
                bkt.ribbon.Button(id=parent_id + "_pos-offset", label="Position offset", screentip="Position numbering shapes offset", on_action=bkt.Callback(self.toggle_pos_offset), get_image=bkt.Callback(lambda: self.get_toggle_image('pos-offset')),
                    supertip="By default, numbering shapes are aligned exactly at the edge of the associated shape.\n\nIf 'Position offset' is enabled, the numbering shapes are placed a bit further outside the associated shape, so that the center of the numbering shape lies on the corner.")
            ],
        )
...
shapes_interactive_menu = memoize(lambda: bkt.ribbon.Menu(
                ...
                id=None, 
                children=[
                    bkt.ribbon.MenuSeparator(title="Insert helpers"),
                    bkt.ribbon.Button(
                        id = 'segmented_circle',
                        label = "Circle segments…",
                        image = "segmented_circle",
                        screentip="Insert circle segments",
                        supertip="Create a circle with segments or chevrons.",
                    ...
                    bkt.ribbon.Menu(
                        label='Graphic tracker',
                        image = "Tracker",
                        screentip="Create or align tracker",
                        supertip="Create, distribute and align a tracker from a selection as an image.",
                        get_enabled = bkt.apps.ppt_shapes_or_text_selected,
                        children = [
                            bkt.ribbon.Button(
                                id = 'tracker',
                                label = "Create tracker from selection",
                                #image = "Tracker",
                                screentip="Create tracker from selection",
                                supertip="Create a tracker from the selected shapes.\nThe shape style for highlights is determined from the first selected shape (usually top left). The shape style for all other shapes is determined from the second selected shape.",
                            ...
                            bkt.ribbon.Button(
                                ...
                                label = "Distribute tracker over slides",
                                #image = "Tracker",
                                screentip="Distribute all trackers",
                                supertip="Distribute the selected trackers over the following slides and align them.",
                            ...
                            bkt.ribbon.Button(
                                id = 'tracker_align',
                                label = "Align all trackers",
                                #image = "Tracker",
                                screentip="Align all trackers",
                                supertip="Align (position, size, rotation) of all trackers (on all slides) based on the selected tracker.",
                            ...
                            bkt.ribbon.Button(
                                ...
                                #image = "Tracker",
                                screentip="Delete all trackers",
                                supertip="Delete all trackers (on all slides) based on the selected tracker.",
                                on_action=bkt.Callback(TrackerShape.removeTracker, shape=True, context=True),
                                get_enabled = bkt.Callback(TrackerShape.isTracker, shape=True),
                            ),
                        ]
                    ),
                    bkt.ribbon.MenuSeparator(title="Interactive shapes"),
                    bkt.ribbon.Button(
                        id = 'standard_process',
                        label = "Process arrows…",
                        image = "process_chevrons",
                        screentip="Insert process arrows",
                        supertip="Create standard process arrows.",
                        on_action=bkt.Callback(ShapeDialogs.show_process_chevrons_dialog)
                    ),
                    bkt.ribbon.Button(
                        id = 'headered_pentagon',
                        label = "Process step with header",
                        image = "headered_pentagon",
                        screentip="Create process-step shape with header",
                        supertip="Create a process arrow with header shape. The header shape can be arranged appropriately within the process arrow via the context menu of the header shape.",
                        on_action=bkt.Callback(ShapeDialogs.create_headered_pentagon)
                    ),
                    bkt.ribbon.Button(
                        id = 'headered_chevron',
                        label = "2nd process step with header",
                    ...
                    bkt.ribbon.Menu(
                        ...
                        label="Traffic light",
                        image="traffic_light",
                        screentip='Create status traffic light',
                        children=[
                            bkt.ribbon.Button(
                                id="traffic_light",
                                label="Traffic light vertical",
                                image="traffic_light",
                                screentip='Create vertical status traffic light',
                                supertip="Insert a status traffic light. The status color of the traffic light can be configured via the context dialog.",
                    ...
                    bkt.ribbon.MenuSeparator(title="Connection areas"),
                    bkt.ribbon.Button(
                        id = 'connector_h',
                        label = "Horizontal connection area",
                        image = "ConnectorHorizontal",
                        supertip="Create a horizontal connection area between the vertical sides (left/right) of two shapes.",
                    ...
                    bkt.ribbon.Button(
                        id = 'connector_v',
                        label = "Vertical connection area",
                        image = "ConnectorVertical",
                        supertip="Create a vertical connection area between the horizontal sides (top/bottom) of two shapes.",
...
class ShapeTableGallery(bkt.ribbon.Gallery):
    ...
    def __init__(self, **kwargs):
        ...
        parent_id = kwargs.get('id') or ""
        my_kwargs = dict(
            label = 'Insert shape table',
            columns = ShapeTableGallery._columns,
            image = 'shapetable',
            # image_mso = 'SlidesPerPage4Slides',
            screentip="Insert shape table",
            supertip="Insert a table of standard shapes",
            description="Insert a table of standard shapes",
            children=[
                bkt.ribbon.Button(id=parent_id + "_margin0", label="Without spacing", supertip="Disable shape-table spacing", on_action=bkt.Callback(lambda: setattr(self, "_margin", 0)), get_image=bkt.Callback(lambda: self.get_toggle_image(0))),
                bkt.ribbon.Button(id=parent_id + "_margin10", label="Small spacing", supertip="Set shape-table spacing to small", on_action=bkt.Callback(lambda: setattr(self, "_margin", 10)), get_image=bkt.Callback(lambda: self.get_toggle_image(10))),
                bkt.ribbon.Button(id=parent_id + "_margin20", label="Large spacing", supertip="Set shape-table spacing to large", on_action=bkt.Callback(lambda: setattr(self, "_margin", 20)), get_image=bkt.Callback(lambda: self.get_toggle_image(20))),
            ]
        )
        my_kwargs.update(kwargs)
...
class ChessTableGallery(ShapeTableGallery):
    ...
    def __init__(self, **kwargs):
        parent_id = kwargs.get('id') or ""
        my_kwargs = dict(
            label = 'Insert shape checkerboard',
            image = 'shapechessboard',
            screentip="Insert shape checkerboard",
            supertip="Insert a checkerboard of standard shapes",
            description="Insert a checkerboard of standard shapes",
        )
        my_kwargs.update(kwargs)
        ...
        self.children.append(
            bkt.ribbon.Button(id=parent_id + "_txtboxes", label="Text boxes in cells", supertip="Set shape-table spacing to large", on_action=bkt.Callback(lambda: setattr(self, "_insert_textboxes", not self._insert_textboxes)), get_image=bkt.Callback(lambda: self.get_check_image(self._insert_textboxes)))
            )
...
shapes_table_menu = memoize(lambda: bkt.ribbon.Menu(
                ...
                id=None, 
                children=[
                    bkt.ribbon.MenuSeparator(title="PowerPoint table"),
                    bkt.mso.control.TableInsertGallery,
                    bkt.ribbon.MenuSeparator(title="Shape table"),
...
shapes_change_menu = memoize(lambda: bkt.ribbon.Menu(
                ...
                id=None, 
                children=[
                    bkt.ribbon.MenuSeparator(title="Manipulate shapes"),
                    bkt.ribbon.Button(
                        label="Split/multiply shapes…",
                        image="split_horizontal",
                        screentip="Split or multiply shapes",
                        supertip="Split or multiply a shape horizontally/vertically into several shapes.",
                    ...
                    bkt.ribbon.Button(
                        ...
                        screentip="Scale shapes",
                        supertip="Change the shape size including all elements/properties (font size, outlines, etc.) uniformly.",
                        on_action=bkt.Callback(ShapeDialogs.shape_scale),
                        get_enabled = bkt.apps.ppt_shapes_or_text_selected,
                    ),
                    bkt.ribbon.Button(
                        label="Convert placeholder into text box",
                        image_mso="ConvertTableToText",
                        supertip="Converts all selected text placeholders into real text boxes that allow grouping, among other things.",
                        on_action=bkt.Callback(PlaceholderConverter.convert_shapes),
                        get_enabled = bkt.apps.ppt_shapes_or_text_selected,
                    ),
                    bkt.mso.control.ObjectEditPoints,
                    bkt.ribbon.Button(
                        label="Convert text/symbol into shapes",
                        image_mso="TextEffectTransformGallery",
                        screentip="Texts or symbols are converted into standard shapes",
                        supertip="Replaces the text of a text box in shapes. This can be used e.g. to convert an icon font into real icons.",
                        on_action=bkt.Callback(ShapesMore.texts_to_shapes),
                        get_enabled = bkt.apps.ppt_shapes_or_text_selected,
                    ),
                    bkt.ribbon.MenuSeparator(title="Merge shapes"),
...
shapes_more_menu = memoize(lambda: bkt.ribbon.Menu(
                ...
                id=None, 
                children=[
                    bkt.ribbon.MenuSeparator(title="Images and objects"),
                    ...
                    bkt.ribbon.MenuSeparator(title="Text & labels"),
                    ...
                    bkt.ribbon.MenuSeparator(title="Show/hide"),
                    bkt.ribbon.Button(
                        id = 'toggle_shapes_visibility',
                        label = "Hide/show shapes",
                        image_mso="ShapesSubtract",
                        supertip="If shapes are selected, hide all selected shapes (visible=False); otherwise make shapes visible again (visible=True). If there are no invisible shapes, the last hidden shapes are hidden again.",
                    ...
                    #     label = u"Hide shapes",
                    #     image_mso="ShapesSubtract",
                    #     supertip="Hide all selected shapes (visible=False).",
                    #     on_action=bkt.Callback(ShapesMore.hide_shapes),
                    #     get_enabled = bkt.apps.ppt_shapes_or_text_selected,
                    # ),
                    bkt.ribbon.Button(
                        id = 'show_shapes',
                        label = "Show all hidden shapes",
                        image_mso="VisibilityVisible",
                        supertip="Make all hidden shapes (visible=False) visible again.",
