...
class ToolboxAgenda(object):
    ...
    @classmethod
    def get_pressed_selector_margin(cls, slide, current_control):
        margin = float(current_control["tag"])
        return cls.get_selector_margin(slide) == margin
    ...
    @classmethod
    def get_check_fillcolor(cls, slide):
        return bkt.ribbon.Gallery.get_check_image(cls.get_selector_fillcolor(slide, "visibility") == 0)

    @classmethod
    def get_check_linecolor(cls, slide):
        return bkt.ribbon.Gallery.get_check_image(cls.get_selector_linecolor(slide, "visibility") == 0)

    @classmethod
    def get_check_textcolor(cls, slide, key="bold", current_control=None):
        if current_control:
            key = current_control["tag"]
        return bkt.ribbon.Gallery.get_check_image(cls.get_selector_textcolor(slide, key))
    ...
    @classmethod
    def get_selector_textcolor(cls, slide, key="color", current_control=None):
        if current_control:
            key = current_control["tag"]
        try:
            settings = cls.get_agenda_settings_from_slide(slide)
    ...
    @classmethod
    def toggle_selector_text_style(cls, slide, style="bold", current_control=None):
        if current_control:
            style = current_control["tag"]
    ...
    @classmethod
    def set_selector_margin(cls, slide, margin=0.2, current_control=None, pressed=False):
        if current_control:
            margin = float(current_control["tag"])
...
agenda_options_menu = lambda: bkt.ribbon.Menu(
                xmlns="http://schemas.microsoft.com/office/2009/07/customui",
                id=None, 
                children=[
                    bkt.ribbon.ToggleButton(
                        id='agenda-slide-for-subitems',
                        label="Agenda slides for sub-items",
                        supertip="Agenda slides are created for sub-items of an agenda item (indent level > 1)",
                    ...
                    bkt.ribbon.ToggleButton(
                        ...
                        label="Hide other agenda sub-items",
                        supertip="Sub-items of an agenda item (indent level > 1) are hidden in the other sections",
                    ...
                    bkt.ribbon.ToggleButton(
                        ...
                        label="Create sections for agenda items",
                        supertip="Begin a new section per agenda slide.",
                    ...
                    bkt.ribbon.ToggleButton(
                        ...
                        label="Create hyperlinks for agenda items",
                        supertip="Link each agenda item to its corresponding agenda slide.",
