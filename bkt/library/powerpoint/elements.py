...
class PositionGallery(bkt.ribbon.Gallery):
    ...
    def __init__(self, positions=None, label="Default positions", columns=3, **kwargs):
        self.items = positions or PositionGallery.items
        super(PositionGallery, self).__init__(
            label = label,
            columns = columns,
            image_mso='PositionAnchoringGallery',
            supertip="Position the selected shapes at a default position.",
            children=[
                bkt.ribbon.Button(
                    label="Set custom area",
                    supertip="The custom area is defined based on the selected shape. This area can then be chosen via the gallery and is kept permanently in the current presentation.",
